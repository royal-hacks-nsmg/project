...
{
  Client client;
  public GenerateContentSimpleText()
  {
  ...
  public EnemyResponseSchema? GenerateContentResponse(string playerInput)
  {
    var response = Task.Run(() => client.Models.GenerateContentAsync(
      model: "gemini-3-flash-preview", contents: SystemPrompt.Value + CharacterBackstory.Value + playerInput
    )).Result;
